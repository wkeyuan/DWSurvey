...
//						quLogicItem.find("input[name='logicEvent']").val(logicEvent);
	...
	$("#dwDialogUeOk").click(function(){
		...
		setCurEditContent(myeditorContent,null);
		curEditCallback();
	...
	$("#dwDialogQuAutoAddZhenColSetSave").click(function(){

		var quItemBody=$(dwDialogObj).parents(".surveyQuItemBody");
		var quOptionParent=$(dwDialogObj).parent();
		var quItemInputCase = quOptionParent.find(".quItemInputCase");
		var quColOption_checkType=$("#modelUIDialog select[name='quColOption_checkType']").val();
		var colCheckType = quItemInputCase.find("input[name='colCheckType']");
		if(!colCheckType[0]){
			quItemInputCase.append("<input type='hidden' name='colCheckType' />");
		}
		quItemInputCase.find("input[name='colCheckType']").val(quColOption_checkType);

		var quColOption_isRequiredFill=$("#modelUIDialog input[name='quColOption_isRequiredFill']:checked");
		var colIsRequiredFill = quItemInputCase.find("input[name='colIsRequiredFill']");
		if(!colIsRequiredFill[0]){
			quItemInputCase.append("<input type='hidden' name='colIsRequiredFill' />");
		}
		if(quColOption_isRequiredFill[0]){
			quItemInputCase.find("input[name='colIsRequiredFill']").val(1);
		}else{
			quItemInputCase.find("input[name='colIsRequiredFill']").val(0);
		}
		quItemInputCase.find("input[name='quItemSaveTag']").val(0);
		quItemBody.find("input[name='saveTag']").val(0);
		$("#modelUIDialog").dialog("close");
		dwCommonDialogHide();
		return false;
	...
	$("#dwDialogQuOptionSetSave").click(function(){
		...
		if(quOption_isNote.prop("checked")){
			...
			// quCoItemUlLi trannoteLi
			quOptionParent.removeClass("trannoteLi");
			quOptionParent.addClass("trannoteLi");
		}else{
			isNote.val(0);
	...
	*/
	...
	$("#previewBtn").click(function(){
		...
		saveSurvey(function(){
			isSaveProgress=false;
			window.location.href=ctx+"/design/spc/dwsurvey-preview/preview.do?surveyId="+questionBelongId;
		});
		return false;
	});
	...
	function saveSurvey(callback){
		...
		if( svyNmSaveTag==="0" || svyNoteSaveTag==="0" || svyAttrSaveTag==="0"){
			var url=ctx+"/design/my-survey-design/ajaxSave.do";
			...
			var ynStartTime=$("input[name='ynStartTime']:checked")[0]?"1":"0";
			...
			data+="&ynStartTime="+ynStartTime+"&startTime="+startTime;
...
function curEditCallback(){
	if(curEditObj!=null){
		...
		var dwEditText=$("#dwComEditContent").text();
		//var curEditObjHtml=$(curEditObj).html();
		setCurEditContent(dwEditHtml,dwEditText);
	}
	$("#dwSurveyNote").removeClass("click");
}

function setCurEditContent(dwEditHtml,dwEditText){

	var thClass=$(curEditObj).attr("class");
	if(dwEditText==null){
		dwEditText = dwEditHtml;
	}
	// alert("is之前："+dwEditHtml+":"+dwEditText);
	if((dwEditText==null || dwEditText== undefined || dwEditText=="") && thClass.indexOf("dwSvyNoteEdit")<0){
		// alert(dwEditHtml);
		deleteDwOption();
...
//		var quItemBody=$(this).parents(".surveyQuItemBody");
		...
		$("#modelUIDialog").dialog("option","height",460);
...
function saveFillblank(quItemBody,callback){
	...
	if(saveTag==0){
		...
		var paramStr01=quItemBody.find("input[name='paramStr01']");
		var paramInt01=quItemBody.find("input[name='paramInt01']");
		...
		if(paramStr01[0]){
			data+="&paramStr01="+paramStr01.val();
		}
		if(paramInt01[0]){
