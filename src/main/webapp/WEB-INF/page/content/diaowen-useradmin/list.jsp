				...
				<div class="surveyCollectMiddleContent">
					<div style="padding: 25px 45px;overflow: auto;padding-top: 12px;">
							...
							<div class="contacts_search" style="padding: 5px;color:#666565;margin-top: 15px;" >
								<form id="formSearch" action="${ctx }/sy/user/user-admin/list.do" method="post">
								<div style="padding-left: 30px;padding-top: 8px;padding-bottom: 8px;">
										<span style="font-size: 14px;vertical-align: middle;">状态&nbsp;</span>
										<select name="status" style="vertical-align: middle;padding: 6px;border: 1px solid #E4DEDE;" >
											<option value="">不限</option>
											<option value="1">可用</option>
											<option value="0">禁用</option>
										</select>&nbsp;&nbsp;
									<span style="font-size: 14px;vertical-align: middle;">用户名&nbsp;</span>
									<input type="text"  name="loginName" value="${loginName }" class="inputS1" style="padding: 6px;" />&nbsp;&nbsp;
									...
									<div style="padding: 5px;color: #666565;text-align: right;float: right;margin-right: 20px;">
										<a href="${ctx }/sy/user/user-admin/input.do" class="user-plus active"><i class="fa fa-plus " aria-hidden="true"></i>&nbsp;添加账号</a>
									</div>
								</div>
								</form>
							</div>
											...
											<c:when test="${page.totalItems > 0}">
												...
												<c:forEach items="${page.result }" var="en">
													<tr>
														...
														<td align="left">${en.status eq 0 ? '禁用':'可用' }</td>
														<td align="center">
															<a class="btn btn-default" href="${ctx }/sy/user/user-admin/input.do?id=${en.id}" title="编辑"data-toggle="tooltip" data-placement="top" ><i class="fa fa-pencil-square-o"></i></a>
															<c:choose>
																<c:when test="${en.status eq 0}">
																	<a class="btn btn-default disUser_a" href="${ctx }/sy/user/user-admin/delete.do?id=${en.id}" title="还原"data-toggle="tooltip" data-placement="top" ><i class="fa fa-reply" aria-hidden="true"></i></a>
																</c:when>
																<c:otherwise>
																	<a class="btn btn-default disUser_a" href="${ctx }/sy/user/user-admin/delete.do?id=${en.id}" title="禁用"data-toggle="tooltip" data-placement="top" ><i class="fa fa-ban" aria-hidden="true"></i></a>
																</c:otherwise>
															</c:choose>
															<a class="btn btn-default disUser_a" href="${ctx }/sy/user/user-admin/delete.do?id=${en.id}&event=delete" title="删除"data-toggle="tooltip" data-placement="top" ><i class="fa fa-trash" aria-hidden="true"></i></a>
														</td>
													</tr>
												</c:forEach>
								...
								<div style="padding-top: 15px;text-align: center;">
									<div class="btn-group">
										<c:if test="${page.pageNo > 1}">
											<a href="${ctx }/sy/user/user-admin/list.do?pageNo=${page.pageNo-1}" class="btn btn-default pageIndex">&lt;</a>
										</c:if>
										<c:if test="${page.startpage > 1}">
											<a href="${ctx }/sy/user/user-admin/list.do?pageNo=1" class="btn btn-default pageIndex">1</a>
										...
										<c:forEach begin="${page.startpage }" end="${page.endpage }" var="en">
											<c:choose>
												<c:when test="${page.pageNo eq en }"><a href="${ctx }/sy/user/user-admin/list.do?pageNo=${en }" class="btn btn-default pageIndex" style="background: #D3DEED;">${en }</a></c:when>
												<c:otherwise><a href="${ctx }/sy/user/user-admin/list.do?pageNo=${en}" class="btn btn-default pageIndex">${en }</a></c:otherwise>
											</c:choose>
										</c:forEach>
										<c:if test="${page.totalPage > (page.endpage)}">
											...
											<a href="${ctx }/sy/user/user-admin/list.do?pageNo=${page.totalPage}" class="btn btn-default pageIndex">${page.totalPage }</a>
										</c:if>
